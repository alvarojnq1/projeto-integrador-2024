...
public class CadastrarProfessor extends JPanel{
    private JFrame CadastrarProfessor;
    private Image imagemDeFundo;
    private Image blocoCadastrarProfessor;
    private ConnectionFactory connectionFactory;
    private JTextField nome;
    private JTextField email;
    private JPasswordField senha;

    public CadastrarProfessor(JFrame CadastrarProfessor){
    ...
    public void carregarImagens(){
        ...
        blocoCadastrarProfessor = new ImageIcon(getClass().getResource("/images/blococadastroprofessor.png")).getImage();
    }
    ...
    public void cadastrar() {
        ...
        String senhaProf = new String(senha.getPassword()); // Recuperando a senha como uma string
    
        // Validar o email
        if (!ValidaEmail.validar(emailProf)) {
        ...
        try {
            // Verificar se o email já existe no banco de dados
            if (emailJaCadastrado(emailProf)) {
        ...
        try (Connection conn = connectionFactory.obtemConexao()) {
            ...
            String sql = "INSERT INTO professores (nome_professor, email_professor, senha_professor) VALUES (?, ?, ?)";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                // Definir os parâmetros da instrução SQL
                stmt.setString(1, nomeProf);
                stmt.setString(2, emailProf);
                stmt.setString(3, senhaProf);
                ...
                if (rowsAffected > 0) {
                    ...
                    JOptionPane.showMessageDialog(this, "Falha ao cadastrar professor.");
                }
            }
        } catch (SQLException ex) {
    ...
    private boolean emailJaCadastrado(String email) throws SQLException {
        ...
        try (Connection conn = connectionFactory.obtemConexao()) {
            ...
            String sql = "SELECT COUNT(*) FROM professores WHERE email_professor = ?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int x = (getWidth() - blocoCadastrarProfessor.getWidth(null)) / 2;
        int y = (getHeight() - blocoCadastrarProfessor.getHeight(null)) / 2;
        g.drawImage(blocoCadastrarProfessor, x, y, blocoCadastrarProfessor.getWidth(null), blocoCadastrarProfessor.getHeight(null),this);
    }
}
